...
import { EarthPhoto, DayPhoto } from "../types/photos";
...
export const useDayPhotos = (from?: Date, to?: Date) => {
    ...
    return useQuery<DayPhoto[], AxiosError<ProblemDetails>>({
        ...
        queryFn: async () => {
            try {
                const response = await axios.get<DayPhoto[]>(`api/photo/apod?from=${from?.toISOString()?.substring(0, 10) ?? ""}&to=${to?.toISOString()?.substring(0, 10) ?? ""}`);
